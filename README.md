...
- 창 이동 및 크기 조절
- 목록이 창 높이를 넘으면 아래쪽에 더 있다는 화살표 표시
- 설정에서 목록 수에 맞춘 창 높이 자동 조절 선택
- 불투명도와 모서리 둥글기 조절
...
압축을 풀고 먼저 `Mac에서 처음 실행.command`를 한 번 실행합니다. 이 파일이 다운로드 격리 속성을 정리하고 Mac의 `codesign`으로 앱을 현재 컴퓨터에서 실행 가능하게 준비한 뒤 열어 줍니다. `.command` 실행이 차단되면 Control 클릭 후 `열기`를 선택하세요. 자세한 내용은 [macOS 실행 안내](docs/macOS-실행.md)를 보세요.
...
- 더보기 → `화면·실행 설정…`: 색상, 불투명도, 모서리, 항상 위, 목록에 맞춘 창 높이 자동 조절, 자동 실행 설정
